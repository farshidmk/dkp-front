...
export const WalletFormValidation = z
  ...
  amount: z
    ...
    .refine((val) => Number(val) <= 100000000, "حداکثر مبلغ ۱۰۰ میلیون تومان است"),
  order_id: z
    .string()
    .optional()
    .refine((val) => val === undefined || val === "" || !isNaN(Number(val)), "کد پیگیری باید عدد باشد")
    .refine((val) => val === undefined || val === "" || Number(val) >= 1, "کد پیگیری باید بزرگتر از صفر باشد")
    .refine((val) => val === undefined || val === "" || Number(val) <= 999999999, "کد پیگیری نباید بیش از ۹ رقم باشد"),
  description: z
    .string()
